...
(async () => {
	...
	const container = await window.sleezzi.awaitForElement("#assetsItems");
	if (!container) return;
	
	await window.sleezzi.awaitForElement("#assetsItems li.list-item.item-card.ng-scope.place-item div.item-card-container"); // Await for items to load.
